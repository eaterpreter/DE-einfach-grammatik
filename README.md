...
> 德語文法教材（繁體中文講解 + 德語例句），可依 Stage／Unit 系統化自學或搭配 AI 助教使用。

## 專案說明

本倉庫是一套**模組化德語文法講義與練習**：每個單元為一個 Markdown 檔，檔名編號為 **`{Stage}-{Unit}-{主題 slug}.md`**，置於 [`units/`](units/) 目錄。講義以繁體中文說明概念，並附足量德語例句與練習。

- **課程藍圖與必教範圍**：見根目錄 [`grammatik-roadmap.md`](grammatik-roadmap.md)（含 Stage 順序、與文法書 Lektion 的核銷對照說明）。
- **產出進度與單元登錄**：見 [`docs/build-progress.md`](docs/build-progress.md) §②（給作者／建置期接手用）。
- **單課格式與題型規格**：見 [`docs/lesson-template.md`](docs/lesson-template.md)、[`docs/exercise-rules.md`](docs/exercise-rules.md)、[`docs/style-guide.md`](docs/style-guide.md)。
- **學習進度紀錄**：見 [`learner/progress.md`](learner/progress.md)（讀書期由 AI 讀此檔接續下一課）。

文法書 **Lektion 1–40** 在本專案中作為**素材與核銷參考**；實際教學順序與拆段以 **roadmap** 與 **build-progress** 為準（一書課可對應多個 unit）。

更完整的使用方式（含**建置期** prompt）見 [`quickstart.md`](quickstart.md)。以下節錄**讀書期**可直接複製的提示詞。

## 讀書期：複製下列 Prompt

### 路徑與工作區

- **建議**：將編輯器工作區設為語言庫根目錄（例如 `D:\Notes`），本專案路徑為其中的 **`DE\`**。
- 下列區塊中，**`D:\Notes`** 版使用絕對路徑；若**只開本 `DE` 資料夾**為工作區，請改用「僅開 DE」版（相對 `@docs\`／`@learner\`）。
...
_現在式規則動詞、疑問句、名詞性／格入門、定冠詞變格（書約 L1–L4）_
...
_不定冠詞變格、*sein／haben／werden*、動詞位置、複數（書約 L5–L8）_
...
_不規則現在式、指示／否定／所有格冠詞、介詞 1–2（書約 L9–L12）_
...
_人稱代詞、情態與感官動詞、*werden* 未來、*nicht*、可分動詞（書約 L13–L16）_
...
_形容詞變格、名詞化、時態總覽、動詞三形（書約 L21–L24）_
...
_Präteritum、Perfekt、Plusquamperfekt／Futur II、時態圖、反身（書約 L25–L28）_
...
_被動、*es*、疑問詞、代名詞、關係子句（書約 L29–L32）_
...
_關係副詞、比較級／*zu*-不定式、分詞（書約 L33–L36）_
...
_語式、Konjunktiv II／I、引語、數詞（書約 L37–L40）_
